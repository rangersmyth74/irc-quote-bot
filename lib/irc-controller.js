...
const MISSING_SERVER = 'Cannot connect to IRC without server name in config';
const MISSING_CHANNELS = 'Cannot connect to IRC without channels in config';

const NO_ADD = 'Unable to add quote';
...
class IrcController {
    constructor(quotes, config, onError) {
        if (!config.server) {
            logger.error(MISSING_SERVER);
            onError();
            return;
        }

        if (!config.channels || !config.channels.length) {
            logger.error(MISSING_CHANNELS);
            onError();
            return;
        }
    ...
    reply(channel, quotes, success, error) {
        if (quotes) {
            ...
            responses.forEach(response => this.client.say(channel, response));
        } else {
    ...
    handleFindQuote(channel, term) {
        this.reply(channel,
            ...
            quotes => {
                ...
                let sample = _.sampleSize(quotes, this.config.quote_limit);
                sample.sort((a, b) => a.index - b.index);
                return sample.map(quote => `(${quote.index + 1}/${quote.total}) [${quote.date}]: ${quote.quote}`);
            },
