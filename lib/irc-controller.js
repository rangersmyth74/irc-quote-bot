...
class IrcController {
    ...
    reply(channel, quotes, success, error) {
        if (quotes) {
            ...
            responses.slice(0, this.config.quote_limit).forEach(response => this.client.say(channel, response));
        } else {
    ...
    handleFindQuote(channel, term) {
        this.reply(channel,
            ...
            quotes => {
                ...
                return quotes.map(quote => `(${quote.index + 1}/${quote.total}) [${quote.date}]: ${quote.quote}`);
            },
